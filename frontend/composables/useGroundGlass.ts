...
export function useGroundGlass(config?: GroundGlassConfig) {
  ...
  // Rest state: random positions with visible opacity
  const restMesh: MeshState = randomMesh()
  const state = reactive<MeshState>({ ...restMesh })
  const rest = { ...restMesh }
  const target = reactive<MeshState>({ ...restMesh })
